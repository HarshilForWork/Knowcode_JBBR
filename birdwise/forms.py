...
class RegisterForm(FlaskForm):
    ...
    def validate_email_address(self, email_address_to_check):
        email_id = User.query.filter_by(email_id=email_address_to_check.data).first()
        if email_id:
            raise ValidationError('Email Address already exists! Please try a different email address')

    actual_name = StringField(label='Enter Your Name', validators=[Length(min=2, max=30), DataRequired()])
